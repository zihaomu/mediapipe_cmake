...
// This class is used to segment hair from the input image.
// ATTETNION: The hair_segmenter.tflite model only works for opencv_lite with tflite backend.
class HairSegmenter
{
public:

    /// create a HairSegmenter instance.
    /// \param path model path.
    /// \param device default CPU device num is 0, GPU is 1.
    HairSegmenter(std::string modelPath, int device = 0);
...
private:
    ...
    std::vector<std::string> outputNameFromModel = {"conv2d_transpose_4"};
